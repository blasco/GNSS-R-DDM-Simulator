...
def main():
    ...
    fig_ddm_deconv, ax_ddm_deconv = plt.subplots(1,figsize=(10, 4))
    plt.title('Deconv')
    plt.xlabel('C/A chips')
    plt.ylabel('Hz')
    contour_rescaled = ax_ddm_deconv.imshow(ddm_deconv, cmap='jet', 
            extent=(
                delay_increment_start/delay_chip, delay_increment_end/delay_chip, 
                doppler_increment_end, doppler_increment_start), 
            aspect='auto'
            )
    fig_ddm_deconv.colorbar(contour_rescaled, label='Correlated Power [Watts]')
